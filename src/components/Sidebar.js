...
function Sidebar({setTitle}) {
  ...
  return (
    <div className="absolute top-0 pr-3 z-100 left-0 w-72 h-[105vh] flex flex-col justify-center items-center rounded-r-3xl bg-[#6B120A]">
      <ul className="overflow-scroll overflow-x-hidden flex flex-col gap-1 justify-center w-72 items-center text-white tracking-wider text-sm">
          <Link to='/Data-Visualization' onClick={()=>{setTitle("Dashboard")}} className={`w-[90%] p-4 rounded-xl  cursor-pointer ${location.pathname==='/'?'bg-[#ef5e5020]':'bg-[#6B120A]'} hover:bg-[#ef5e5020] flex justify-center items-center gap-5`}>
          <span class="material-symbols-outlined">dashboard</span>
          <p className="font-bold ">Dashboard</p>
          </Link>
        <Link to='/analytics' onClick={()=>{setTitle("Analytics")}} className={`w-[90%] p-4 rounded-xl  cursor-pointer ${location.pathname==='/analytics'?'bg-[#ef5e5020]':'bg-[#6B120A]'} hover:bg-[#ef5e5020] flex justify-center items-center gap-5`}>
